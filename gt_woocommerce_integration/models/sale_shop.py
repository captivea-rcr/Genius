...
class SaleShop(models.Model):
    ...
    def create_woocom_product(self, product, wcapi):
        ...
        prd_tmp_vals = {
            ...
            'product_wght': product.get('dimensions') and product.get('dimensions').get('weight') or False,
        }
        ...
        if images_list:
            ...
            # prd_tmp_vals.update({'woocom_product_img_ids':img_ids})
                try:
                    ...
                    prd_tmp_vals.update({'image_medium':img})
                    f.close()
                except:
                    pass
            img_ids.append((0, 0, img_vals))
        ...
        if temp_ids:
            ...
            if at_lines:
                for variant_data in at_lines:
                    p_at_ids = product_att_line_obj.search([('attribute_id', '=', variant_data[2].get('attribute_id')),
                    ...
                    if p_at_ids:
                        ...
                        for vd in variant_data[2].get('value_ids')[0][2]:
                            if vd not in v_data:
                                new_vals.append(vd)
                        if new_vals:
                            new_lines.append((1, p_at_ids[0].id, {'attribute_id': variant_data[2].get('attribute_id'),
...
#            print('----------',product_list)
    ...
    def create_woo_customer(self, customer_detail, wcapi):
        ...
        account_receive_id = self.env['account.account'].search([('user_type_id.type','=','receivable'),('company_id','=',self.env.user.company_id.id)])
        account_pay_id = self.env['account.account'].search([('user_type_id.type','=','payable'),('company_id','=',self.env.user.company_id.id)])
...
#            Customize code Dee
            ...
            if customer_detail.get('billing').get('company') and customer_detail.get('billing').get('company')!='False':
...
#            uuuuuuuuuuuuuuuuuuuuuuu
            ...
            if customer_detail.get('shipping').get('city'):
                name=''
                if customer_detail.get('shipping').get('first_name') and customer_detail.get('shipping').get('last_name'):
                    name+=customer_detail.get('shipping').get('first_name')+' '+customer_detail.get('shipping').get('last_name')
...
#                Customized code Dee
            ...
            if child.get('subtotal_tax') != None:
                ...
                if tax_id[0]:
                    line['tax_id'] = []# Removed the tax part from here [(6, 0, tax_id)]
...
#            Customize code Dee
            ...
            if order_detail.get('billing').get('first_name') and order_detail.get('billing').get('last_name'):
                name+=order_detail.get('billing').get('first_name')+' '+order_detail.get('billing').get('last_name')
...
#                print('==================')
            ...
            if order_detail.get('shipping'):
                ...
                if order_detail.get('shipping').get('first_name') and order_detail.get('shipping').get('last_name'):
                    name+=order_detail.get('shipping').get('first_name')+' '+order_detail.get('shipping').get('last_name')
...
#            partner_id = self[0].partner_id.id
            guest_user=res_partner_obj.search([('email','=',order_detail.get('billing').get('email')),('parent_id.name','=',order_detail.get('billing').get('company'))],limit=1)
            
            if not guest_user:
                partner_id=self.create_guest_customer(order_detail, wcapi)[0].id
            else:
                partner_id=guest_user.id
        ...
        sale_order_ids = sale_order_obj.search([('woocom_id', '=', order_detail.get('id'))],limit=1)
    ...
    def importWoocomOrder(self):
        ...
        for shop in self:
            ...
            if self.env.context.get('last_woocommerce_order_import_date'):
                method = "orders?filter[created_at_min]=" + str(self.env.context.get('last_woocommerce_order_import_date'))
    ...
    def importRefundOrder(self):
        ...
        for shop in self:
            ...
            if self.env.context.get('last_woocommerce_refund_order_import_date'):
                method = "orders?filter[created_at_min]=" + str(self.env.context.get('last_woocommerce_refund_order_import_date'))
            ...
            for product in product_ids:
                ...
                if product.attribute_line_ids:
                    ...
                    if attributes:
                        ...
                        print("prod_export_resprod_export_resprod_export_res2222222",prod_export_res_attr,prod_export_res_attr_d)
                        if prod_export_res_attr:
                            product.write({'woocom_id': prod_export_res_attr.get('id'), 'product_to_be_exported': False})
                        ...
                        for variant in prod_var_id:
                            ...
                            variation_vals = {
                                ...
                                'attributes': [{'option': avalue.name, 'name': avalue.attribute_id.name} for avalue in
